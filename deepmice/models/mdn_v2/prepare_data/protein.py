...
def from_pdb_path(pdb_path):
    pdb_str = remove_altloc(pdb_path)
    return from_pdb_string(pdb_str)
...
def from_pdb_string(pdb_str: str, chain_id: Optional[str] = None) -> Protein:
    ...
    for chain in model:
        if chain_id is not None and chain.id != chain_id or len(chain.id.strip()) == 0:
            continue
        for res in chain:
            # if res.id[2] != ' ':
            #   raise ValueError(
            ...
            atypes = ['other' for i in range(residue_constants.atom_type_num)]
            ...
            for atom in res:
                ...
                atypes[residue_constants.atom_order[atom.name]] = atom.name[0]
...
def to_pdb(prot: Protein) -> str:
    ...
    def res_1to3(r): return residue_constants.restype_1to3.get(restypes[r], 'UNK')
